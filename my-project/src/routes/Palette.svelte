...
<div class="Container CenterBottom">
    <div class="flex justify-between pb-4">
        <div class="Header font-bold uppercase py-2 px-4 bg-white border-2 rounded-lg">{ActivePalette[0]}</div>
        <div class="Header font-bold uppercase py-2 px-4 bg-white border-2 rounded-lg">20/20/2000</div>
    </div>
    

    <div class="flex justify-between items-center py-4 px-6 bg-white border-2 rounded-lg">
        <button class=" mr-4 rounded-full p-3 border-2 active:opacity-50 hover:opacity-80 duration-200" style:background-color={$ActiveColours[4]} disabled={Count == 0} on:click={() => Decrease()}>
        ...
        {#each $ActiveColours as Colour}
            <button style:background-color={Colour} on:click={() => {CopyPalette(Colour)}} class="w-24 h-16 relative hover:-translate-y-2 duration-200 group mx-2 rounded-lg border-2 active:opacity-50 active:border-green-700">
                ...
                <div class="bg-black bg-opacity-15 absolute bottom-0 opacity-0 p-1 group-hover:opacity-100 duration-200">
                    <div class="text text-white font-bold uppercase">{Colour}</div>
                </div>
            </button>
        {/each}

        <button class="p-3 ml-4 rounded-full border-2 active:opacity-50 hover:opacity-80 duration-200" style:background-color={$ActiveColours[4]} disabled={Count == $TablePalette.length -1} on:click={() => Increase()}>
